...
void sithRender_RenderDebugLight2(flex_t intensity, rdVector3 *pos,
                                  rdVector3 *norm) {
#if 0
    rdVector3 scale_test;
...
void sithRender_RenderDebugLights() {
  ...
  for (int k = 0; k < sithRender_numSectors; k++) {
    ...
    for (int i = 0; i < rdCamera_pCurCamera->numLights; i++) {
      sithRender_RenderDebugLight(rdCamera_pCurCamera->lights[i]->intensity,
                                  &rdCamera_pCurCamera->lightPositions[i]);

      /*flex_t distCalc =
      rdVector_Dist3(&rdCamera_pCurCamera->lightPositions[i],
      &sectorIter->center); if ( (*curCamera_lights)->falloffMin +
      sectorIter->radius > distCalc)
      {
    ...
    {
        int idx = *verticeIdxs;
        if ( sithWorld_pCurrentWorld->alloc_unk9c[idx] !=
    sithRender_lastRenderTick )
        {
            ...
            for (int i = 0; i < v24; i++)
            {
                ...
                flex_t distCalc =
    rdVector_Dist3(&rdCamera_pCurCamera->lightPositions[id],
    &sithWorld_pCurrentWorld->vertices[idx]); if ( distCalc <
    (*lightIter2)->falloffMax )
                    sithWorld_pCurrentWorld->verticesDynamicLight[idx] =
    (*lightIter2)->intensity - distCalc * rdCamera_pCurCamera->attenuationMax +
    sithWorld_pCurrentWorld->verticesDynamicLight[idx]; if (
    sithWorld_pCurrentWorld->verticesDynamicLight[idx] >= 1.0 ) break;
                ++lightIter2;
            }
            sithWorld_pCurrentWorld->alloc_unk9c[idx] =
    sithRender_lastRenderTick;
        }
        verticeIdxs++;
    }*/
...
int sithRender_Open() {
  ...
  sithRenderSky_Open(sithWorld_pCurrentWorld->horizontalPixelsPerRev,
                     sithWorld_pCurrentWorld->horizontalDistance,
                     sithWorld_pCurrentWorld->ceilingSky);
...
void sithRender_SetSomeRenderflag(int flag) { sithRender_flag = flag; }

int sithRender_GetSomeRenderFlag() { return sithRender_flag; }
...
void sithRender_DisableIRMode() { sithRender_lightingIRMode = 0; }
...
void sithRender_Draw() {
  ...
  // printf("%x %x %x\n", sithRender_texMode, rdroid_curTextureMode,
  // sithRender_lightMode);
...
#ifdef TARGET_N64
    static int _logged = 0; if (!_logged) { debugf("[N64] sithRender_Draw: early exit geoMode=0\n"); _logged=1; }
#endif
    return;
  }
  ...
  if (!sithCamera_currentCamera || !sithCamera_currentCamera->sector) {
#ifdef TARGET_N64
    {
      ...
      if (_now - _last_log > 1000) {
        _last_log = _now;
        sithThing* _ft = sithCamera_currentCamera ? sithCamera_currentCamera->primaryFocus : NULL;
        debugf("[N64] sithRender_Draw: sector=NULL cam=%p focus=%p focus_sector=%p g_submodeFlags=0x%x\n",
               sithCamera_currentCamera, _ft, _ft ? _ft->sector : NULL, g_submodeFlags);
      }
    }
#endif
    return;
  ...
  sithPlayer_SetScreenTint(sithCamera_currentCamera->sector->tint.x,
                           sithCamera_currentCamera->sector->tint.y,
                           sithCamera_currentCamera->sector->tint.z);
  ...
  if ((sithCamera_currentCamera->sector->flags & 2) != 0) {
    ...
    rdCamera_SetAspectRatio(&sithCamera_currentCamera->rdCam,
                            a3 * 0.016666668 + aspect);
    sithRender_needsAspectReset = 1;
  } else if (sithRender_needsAspectReset) {
    rdCamera_SetFOV(&sithCamera_currentCamera->rdCam,
                    sithCamera_currentCamera->fov);
    rdCamera_SetAspectRatio(&sithCamera_currentCamera->rdCam,
                            sithCamera_currentCamera->aspectRatio);
    sithRender_needsAspectReset = 0;
  }
#endif
  ...
  if (!sithPlayer_bNoClippingRend) {
    // sithRender_Clip(sithCamera_currentCamera->sector,
    // rdCamera_pCurCamera->pClipFrustum, 0.0, 0);
    sithRender_flag |= 4;
#ifdef TARGET_N64
    ...
    sithRender_KindaClipAssignFrustum(sithCamera_currentCamera->sector,
                                      rdCamera_pCurCamera->pClipFrustum, 0, 0);
    sithRender_KindaClip(sithCamera_currentCamera->sector,
                         rdCamera_pCurCamera->pClipFrustum, 0.0, 0);
    sithRender_flag &= ~4;
  } else {
    sithPlayer_bNoClippingRend = 0;
    ...
    sithRender_NoClip(sithCamera_currentCamera->sector,
                      rdCamera_pCurCamera->pClipFrustum, 0.0, 0);
    sithRender_flag &= ~4;
    ...
    for (int i = 0; i < sithWorld_pCurrentWorld->numSectors; i++) {
      ...
      if (pSectorIter->clipVisited == sithRender_lastRenderTick ||
          pSectorIter->renderTick == sithRender_lastRenderTick) {
        continue;
      }
      /*flex_t dist =
      rdMath_DistancePointToPlane(&sithCamera_currentCamera->vec3_1,
      &rdCamera_pCurCamera->view_matrix.lvec, &pSectorIter->center); if (dist +
      (pSectorIter->radius * 3.5) < 0.0) { continue;
      }
      ...
      rdMatrix_TransformPoint34Acc(&centerTrans,
                                   &rdCamera_pCurCamera->view_matrix);
      int clipTestA = rdClip_SphereInFrustum(rdCamera_pCurCamera->pClipFrustum,
                                             &centerTrans, 0.0);
      int clipTestB = rdClip_SphereInFrustum(rdCamera_pCurCamera->pClipFrustum,
                                             &centerTrans, pSectorIter->radius);
  ...
  if (!sithPlayer_bNoClippingRend) {
    ...
    for (int i = 0; i < sithWorld_pCurrentWorld->numSectors; i++) {
      ...
      /*flex_t dist =
      rdMath_DistancePointToPlane(&sithCamera_currentCamera->vec3_1,
      &rdCamera_pCurCamera->view_matrix.uvec, &pSectorIter->center); if (dist +
      (pSectorIter->radius * 3.5) < 0.0) { continue;
      }
      ...
      rdMatrix_TransformPoint34Acc(&centerTrans,
                                   &rdCamera_pCurCamera->view_matrix);
      if (rdClip_SphereInFrustum(rdCamera_pCurCamera->pClipFrustum,
                                 &centerTrans, pSectorIter->radius * 3.5) ==
          SPHERE_FULLY_OUTSIDE) {
        flex_t dist = rdVector_Dist3(&sithCamera_currentCamera->vec3_1,
                                     &pSectorIter->center);
  ...
  if (sithRender_008d4094 != 0) {
    ...
    flex_t fVar3 = sithRender_008d4098 - (flex_t)local_8 * sithRender_008d409c *
                                             sithTime_deltaSeconds;
    if (0.0 <= sithRender_008d4098) {
      if (sithRender_008d4098 < 0.0) {
        iVar6 = 0;
      } else {
...
#include <libdragon.h>
  ...
  extern void consoleGetCursor(int *x, int *y);
  consoleGetCursor(&consoleX, &consoleY);
  snprintf(resetConsole, sizeof(resetConsole) - 1, "\x1b[%d;%dH", consoleY,
           consoleX);
  printf("\x1b[8;0H                               \rclp=%d lts=%d geo=%d "
         "thg=%d al=%d %d \n                        \n",
         testClipEnd - testClip, testLightsEnd - testLights,
         testLevelGeoEnd - testLevelGeo, testThingsEnd - testThings,
         testAlphaEnd - testAlpha, (int)sithRender_numSectors);
  stdPlatform_Printf(resetConsole);
#else
  debugf("clp=%d lts=%d geo=%d thg=%d al=%d %d\n",
         testClipEnd - testClip, testLightsEnd - testLights,
         testLevelGeoEnd - testLevelGeo, testThingsEnd - testThings,
         testAlphaEnd - testAlpha, (int)sithRender_numSectors);
#endif
...
// MOTS altered?
// Added: depth safety
void sithRender_Clip(sithSector *sector, rdClipFrustum *frustumArg,
                     flex_t prevAdjoinDistAdd, int depth) {
  int v5;                     // ecx
  ...
  if (sector->renderTick == sithRender_lastRenderTick) {
    ...
    if (!(sector->flags & SITH_SECTOR_AUTOMAPVISIBLE) &&
        !(g_debugmodeFlags &
          DEBUGFLAG_NOCLIP)) // Added: don't send sighted stuff in noclip,
                             // otherwise the whole map reveals
    {
    ...
    while (thing) {
      ...
      if ((thing->thingflags & SITH_TF_LIGHT) &&
          !(thing->thingflags &
            (SITH_TF_DISABLED | SITH_TF_10 | SITH_TF_WILLBEREMOVED))) {
        if (thing->light > 0.0) {
          sithRender_aLights[lightIdx].intensity = thing->light;
          rdCamera_AddLight(rdCamera_pCurCamera, &sithRender_aLights[lightIdx],
                            &thing->position);
          lightIdx = ++sithRender_numLights;
        }
        ...
        if ((thing->type == SITH_THING_ACTOR ||
            ...
            lightIdx < 0x20) {
          if ((thing->actorParams.typeflags & SITH_AF_FIELDLIGHT) != 0 &&
              thing->actorParams.lightIntensity > 0.0) {
            rdMatrix_TransformPoint34(&vertex_out,
                                      &thing->actorParams.lightOffset,
                                      &thing->lookOrientation);
            rdVector_Add3Acc(&vertex_out, &thing->position);
            sithRender_aLights[sithRender_numLights].intensity =
                thing->actorParams.lightIntensity;
            rdCamera_AddLight(rdCamera_pCurCamera,
                              &sithRender_aLights[sithRender_numLights],
                              &vertex_out);
            lightIdx = ++sithRender_numLights;
          }
          if (thing->actorParams.timeLeftLengthChange > 0.0) {
            sithRender_aLights[lightIdx].intensity =
                thing->actorParams.timeLeftLengthChange;
            rdCamera_AddLight(rdCamera_pCurCamera,
                              &sithRender_aLights[lightIdx],
                              &thing->actorParams.saberBladePos);
            lightIdx = ++sithRender_numLights;
          }
        }
      }
  ...
  // Added: safeguard
  for (adjoinIter = sector->adjoins; adjoinIter != NULL;
    ...
    v20 = &sithWorld_pCurrentWorld
               ->vertices[*adjoinSurface->surfaceInfo.face.vertexPosIdx];
    flex_t dist = rdMath_DistancePointToPlane(
        &sithCamera_currentCamera->vec3_1,
        &adjoinSurface->surfaceInfo.face.normal, v20);
    flex_t adjoinDistAdd =
        adjoinIter->dist + adjoinIter->mirror->dist + prevAdjoinDistAdd;
    ...
    if (dist > 0.0 ||
        (dist == 0.0 && sector == sithCamera_currentCamera->sector)) {
      int bAdjoinIsTransparent =
          (((!adjoinSurface->surfaceInfo.face.material ||
           ...
           (v51 && (v51->header.texture_type & 8) &&
            (v51->texture_ptr &&
             v51->texture_ptr->alpha_en & 1)) // Added: v51->texture_ptr check
          );

#ifdef QOL_IMPROVEMENTS
      // Added: Somehow the clipping changed enough to cause a bug in MoTS Lv12.
      // The ground under the water surface somehow renders.
      // As a mitigation, if a mirror surface is transparent but the top-layer
      // isn't, we will render underneath anyways.
      sithSurface *adjoinMirrorSurface = adjoinIter->mirror->surface;
      rdMaterial *adjoinMirrorMat =
          adjoinMirrorSurface->surfaceInfo.face.material;
      ...
      if (adjoinMirrorMat) {
        ...
        adjoinMirrorTexinfo =
            NULL; // Added. TODO: does setting this to NULL cause issues?
      }

      int bMirrorAdjoinIsTransparent =
          (((!adjoinMirrorSurface->surfaceInfo.face.material ||
             (adjoinMirrorSurface->surfaceInfo.face.geometryMode ==
              RD_GEOMODE_NOTRENDERED)) ||
            ((adjoinMirrorSurface->surfaceInfo.face.type & 2))) ||
           (adjoinMirrorTexinfo &&
            (adjoinMirrorTexinfo->header.texture_type & 8) &&
            (adjoinMirrorTexinfo->texture_ptr &&
             adjoinMirrorTexinfo->texture_ptr->alpha_en & 1)));
      ...
      if (adjoinSurface->field_4 != sithRender_lastRenderTick) {
        for (int i = 0; i < adjoinSurface->surfaceInfo.face.numVertices; i++) {
          ...
          if (sithWorld_pCurrentWorld->alloc_unk98[v25] !=
              sithRender_lastRenderTick) {
            rdMatrix_TransformPoint34(
                &sithWorld_pCurrentWorld->verticesTransformed[v25],
                &sithWorld_pCurrentWorld->vertices[v25],
                &rdCamera_pCurCamera->view_matrix);
            sithWorld_pCurrentWorld->alloc_unk98[v25] =
                sithRender_lastRenderTick;
          }
        }
      ...
      sithRender_idxInfo.vertices =
          sithWorld_pCurrentWorld->verticesTransformed;
      sithRender_idxInfo.vertexUVs = sithWorld_pCurrentWorld->vertexUVs;
      sithRender_idxInfo.paDynamicLight =
          sithWorld_pCurrentWorld->verticesDynamicLight;
      sithRender_idxInfo.numVertices =
          adjoinSurface->surfaceInfo.face.numVertices;
      sithRender_idxInfo.vertexPosIdx =
          adjoinSurface->surfaceInfo.face.vertexPosIdx;
      meshinfo_out.vertices = sithRender_aVerticesTmp;
      sithRender_idxInfo.vertexUVIdx =
          adjoinSurface->surfaceInfo.face.vertexUVIdx;

      rdPrimit3_ClipFace(frustumArg, RD_GEOMODE_WIREFRAME, RD_LIGHTMODE_NOTLIT,
                         RD_TEXTUREMODE_AFFINE, &sithRender_idxInfo,
                         &meshinfo_out,
                         &adjoinSurface->surfaceInfo.face.clipIdk);

      if ((((unsigned int)meshinfo_out.numVertices >= 3u) ||
           (rdClip_faceStatus & CLIPSTAT_NONE_VISIBLE)) &&
          ((rdClip_faceStatus & (CLIPSTAT_NEAR | CLIPSTAT_NONE_VISIBLE)) ||
           ((adjoinIter->flags & 1) && bAdjoinIsTransparent))) {
#if defined(TARGET_TWL) || defined(TARGET_N64)
        rdCamera_pCurCamera->fnProjectLstClip(sithRender_aVerticesTmp_projected,
                                              sithRender_aVerticesTmp,
                                              meshinfo_out.numVertices);
#else
        rdCamera_pCurCamera->fnProjectLst(sithRender_aVerticesTmp_projected,
                                          sithRender_aVerticesTmp,
                                          meshinfo_out.numVertices);
#endif
        ...
        if (rdClip_faceStatus & (CLIPSTAT_NEAR | CLIPSTAT_NONE_VISIBLE)) {
          ...
          rdCamera_BuildClipFrustum(rdCamera_pCurCamera, &outClip,
                                    (int)(v46 - -0.5), (int)(v47 - -0.5),
                                    (int)v48, (int)v49);
          v31 = &outClip;
        ...
        if (!(sithRender_flag & 4) || adjoinDistAdd < sithRender_f_82F4B0) {
          if (depth > 2) {
            sithRender_KindaClip(adjoinIter->sector, v31, adjoinDistAdd,
                                 depth + 1);
          } else {
...
void sithRender_NoClip(sithSector *sector, rdClipFrustum *frustumArg,
                       flex_t prevAdjoinDistAdd, int depth) {
  int v5;                     // ecx
  rdClipFrustum *frustum;     // edx
  sithThing *thing;           // esi
  ...
  if (sector->renderTick == sithRender_lastRenderTick) {
    ...
    // sithRender_lastRenderTick, depth);
  ...
  for (adjoinIter = sector->adjoins; adjoinIter != NULL;
    ...
    // adjoinDistAdd compare GREATLY reduces recursion issues
    // TODO: Test against TODOA and verify if this is QOL-worthy
    if (/*(adjoinDistAdd > 3.5) ||*/ (
        dist > SITHCAMERA_ZFAR) /*|| dist < sector->clipFrustum->zNear*/) {
      // Doesn't help, causes visual issues
      // adjoinIter->sector->clipVisited = sithRender_lastRenderTick;
...
void sithRender_KindaClipAssignFrustum(sithSector *sector,
                                       rdClipFrustum *frustumArg, int depth,
                                       int parentSector) {
  int v5;                     // ecx
  rdClipFrustum *frustum;     // edx
  sithThing *thing;           // esi
...
void sithRender_KindaClip(sithSector *sector, rdClipFrustum *frustumArg,
                          flex_t prevAdjoinDistAdd, int depth) {
  int v5;                     // ecx
  rdClipFrustum *frustum;     // edx
  sithThing *thing;           // esi
  ...
  for (adjoinIter = sector->adjoins; adjoinIter != NULL;
    ...
    if (dist > 0.0 ||
      ...
      if (LIKELY((adjoinIter->flags & 1) && bAdjoinIsTransparent)) {
        ...
        rdClipFrustum *pSphereFrustum =
            frustumArg; // rdCamera_pCurCamera->pClipFrustum;//frustumArg;
        rdMatrix_TransformPoint34Acc(&centerTrans,
                                     &rdCamera_pCurCamera->view_matrix);

        int clipResult =
            rdClip_SphereInFrustum(pSphereFrustum, &centerTrans, radius);

        // Try to guess if the sphere is actually encapsulating the camera
        if (UNLIKELY((dist - radius) < 0.0 ||
                     (dist + radius) >=
                         rdCamera_pCurCamera->pClipFrustum->zFar ||
                     (radius * 2.0) >= 2.5 || dist < 0.01)) {
        ...
        // sithRender_RenderDebugLight2(adjoinSurface->radius * 50.0,
        // &adjoinSurface->center, &adjoinSurface->surfaceInfo.face.normal);
        ...
        if (LIKELY(adjoinSurface->field_4 != sithRender_lastRenderTick)) {
          for (int i = 0; i < adjoinSurface->surfaceInfo.face.numVertices;
               i++) {
            v25 = adjoinSurface->surfaceInfo.face.vertexPosIdx[i];
            if (LIKELY(sithWorld_pCurrentWorld->alloc_unk98[v25] !=
                       sithRender_lastRenderTick)) {
              rdMatrix_TransformPoint34(
                  &sithWorld_pCurrentWorld->verticesTransformed[v25],
                  &sithWorld_pCurrentWorld->vertices[v25],
        ...
        flex_t viewportWidth =
            rdCamera_pCurCamera->canvas->half_screen_width * 2;
        flex_t viewportHeight =
            rdCamera_pCurCamera->canvas->half_screen_height * 2;
        flex_t minX = FLT_MAX;
        flex_t minY = FLT_MAX;
        flex_t maxX = -FLT_MAX;
        flex_t maxY = -FLT_MAX;
        ...
        if (UNLIKELY(adjoinIter->sector->renderTick ==
                     sithRender_lastRenderTick)) {
          leftLimit =
              stdMath_Min(adjoinIter->sector->clipFrustum->farLeft, leftLimit);
          rightLimit =
              stdMath_Max(adjoinIter->sector->clipFrustum->right, rightLimit);
          bottomLimit =
              stdMath_Min(adjoinIter->sector->clipFrustum->bottom, bottomLimit);
          topLimit =
              stdMath_Max(adjoinIter->sector->clipFrustum->farTop, topLimit);
        }
        ...
        if (UNLIKELY(minX == maxX || minY == maxY)) {
          *v31 = *rdCamera_pCurCamera->pClipFrustum;
          ...
          if (UNLIKELY(adjoinIter->sector->renderTick ==
            ...
            sithRender_KindaClip(adjoinIter->sector,
                                 adjoinIter->sector->clipFrustum, adjoinDistAdd,
                                 depth + 1);
            adjoinIter->sector->clipVisited = sithRender_lastRenderTick;
          }
        } else {
        ...
        if (UNLIKELY(adjoinIter->sector->renderTick ==
          ...
          // (flex32_t)adjoinIter->sector->clipFrustum->farLeft,
          // (flex32_t)adjoinIter->sector->clipFrustum->right,
          // (flex32_t)adjoinIter->sector->clipFrustum->bottom,
          // (flex32_t)adjoinIter->sector->clipFrustum->farTop,
          // (flex32_t)v31->farLeft, (flex32_t)v31->right,
          // (flex32_t)v31->bottom, (flex32_t)v31->farTop);

          //*adjoinIter->sector->clipFrustum =
        ...
        sithRender_KindaClipAssignFrustum(adjoinIter->sector, v31, depth + 1,
                                          sector->id);
        sithRender_KindaClip(adjoinIter->sector,
                             adjoinIter->sector->clipFrustum, adjoinDistAdd,
                             depth + 1);
...
void sithRender_RenderLevelGeometry() {
  ...
  sithRender_flag &=
      ~0x8; // Drops render time by 2/3 by rendering by n-gons instead of tris
#endif
  ...
  // printf("%x %x %x %x\n", rdroid_curVertexColorMode, sithRender_flag,
  // rdroid_curAcceleration, sithRender_lightMode);
#endif
  ...
  for (v72 = 0; v72 < sithRender_numSectors; v72++) {
    ...
    if (UNLIKELY(sithRender_lightingIRMode)) {
      ...
      flex_t baseLight =
          level_idk->ambientLight + level_idk->extraLight + sithRender_008d4098;
      ...
      rdSetProcFaceUserData(level_idk->id);    v65 = level_idk->surfaces;

#if defined(TARGET_TWL) || defined(TARGET_N64) ||                              \
    defined(SITHRENDER_SPHERE_TEST_SURFACES) ||                                \
    defined(EXPERIMENTAL_FIXED_POINT)
    ...
    for (v75 = 0; v75 < level_idk->numSurfaces;
         /*v65->field_4 = sithRender_lastRenderTick,*/ ++v65, v75++) {
      ...
      BOOL bIsSkySurface = (v65->surfaceFlags & (SITH_SURFACE_CEILING_SKY |
                                                 SITH_SURFACE_HORIZON_SKY));
      flex_t dist = rdMath_DistancePointToPlane(
          &sithCamera_currentCamera->vec3_1, &v65->surfaceInfo.face.normal,
          &vertices_alloc[*v65->surfaceInfo.face.vertexPosIdx]);
...
#if !defined(TARGET_TWL) && !defined(TARGET_N64)
      if (v65->adjoin && surfaceMat &&
          ((v65->surfaceInfo.face.type & 2) != 0 ||
           (v10 && (v10->header.texture_type & 8)) &&
               (v10 && v10->texture_ptr &&
                (v10->texture_ptr->alpha_en &
                 1)))) // Added: nullptr checks for v10 and v10->texture_ptr
      {
      ...
      if (LIKELY(/*noDistCulling &&*/ !bIsSkySurface)) {
        ...
        clipResult =
            rdClip_SphereInFrustum(pSphereFrustum, &centerTrans, v65->radius);

        /*if (sithRender_lastRenderTick & 1) {
            clipResult = SPHERE_CLIPPING_EDGE;
        }*/
        if (UNLIKELY(v65->radius * 2.0 >
                     rdCamera_pCurCamera->pClipFrustum->zFar)) {
          clipResult = SPHERE_CLIPPING_EDGE;
          ...
          clipResult = rdClip_SphereInFrustum(&sithRender_absoluteMaxFrustum,
                                              &centerTrans, v65->radius);
        }
      ...
      if (LIKELY(v65->field_4 != sithRender_lastRenderTick)) {
        for (int j = 0; j < v65->surfaceInfo.face.numVertices; j++) {
          ...
          if (LIKELY(sithWorld_pCurrentWorld->alloc_unk98[idx] !=
                     sithRender_lastRenderTick)) {
            rdMatrix_TransformPoint34(
                &sithWorld_pCurrentWorld->verticesTransformed[idx],
                &sithWorld_pCurrentWorld->vertices[idx],
                &rdCamera_pCurCamera->view_matrix);
            sithWorld_pCurrentWorld->alloc_unk98[idx] =
                sithRender_lastRenderTick;
          }
        }
      ...
      // Render with N-Gons instead of triangle strips if flag 0x8 is unset, or
      // if it's sky vertices
      if (LIKELY((sithRender_flag & 8) == 0 ||
                 v65->surfaceInfo.face.numVertices <= 3 || bIsSkySurface ||
                 !v65->surfaceInfo.face.lightingMode)) {
        ...
        if (rdGetVertexColorMode() == 0) {
...
#endif
            rdPrimit3_ClipFace(pSurfaceFrustum, procEntry->geometryMode,
                               procEntry->lightingMode, texMode3,
                               &sithRender_idxInfo, &meshinfo_out,
          ...
          if ((v65->surfaceFlags & SITH_SURFACE_1000000) == 0) {
            ...
            sithRender_idxInfo.paRedIntensities =
                (v65->surfaceInfo).intensities + sithRender_idxInfo.numVertices;
          ...
          if (clipResult != SPHERE_FULLY_INSIDE) {
#endif
            rdPrimit3_ClipFaceRGBLevel(pSurfaceFrustum, procEntry->geometryMode,
                                       procEntry->lightingMode, texMode3,
                                       &sithRender_idxInfo, &meshinfo_out,
                                       &(v65->surfaceInfo).face.clipIdk);
#ifdef SITHRENDER_SPHERE_TEST_SURFACES
            ...
            rdPrimit3_ClipFaceRGBLevel(
                rdCamera_pCurCamera->pClipFrustum, procEntry->geometryMode,
                procEntry->lightingMode, texMode3, &sithRender_idxInfo,
                &meshinfo_out, &(v65->surfaceInfo).face.clipIdk);

            // TODO: Bugged?
            /*rdPrimit3_NoClipFaceRGB
            ( // pSurfaceFrustum,            procEntry->geometryMode,
        ...
        rdCamera_pCurCamera->fnProjectLst(procEntry->vertices,
                                          sithRender_aVerticesTmp,
                                          meshinfo_out.numVertices);
        ...
        if (UNLIKELY(sithRender_lightingIRMode)) {
          ...
          procEntry->ambientLight = stdMath_Clamp(
              level_idk->extraLight + sithRender_008d4098, 0.0, 1.0);
        }
        ...
        if (procEntry->ambientLight >= 1.0) {
          ...
          if (procEntry->light_level_static >= 1.0 && v68) {
            ...
            procEntry->lightingMode = RD_LIGHTMODE_NOTLIT;
                   ...
                   procEntry->lightingMode == RD_LIGHTMODE_GOURAUD) {
        ...
        if (UNLIKELY(surfaceFlags & SITH_SURFACE_HORIZON_SKY)) {
          sithRenderSky_TransformHorizontal(procEntry, &v65->surfaceInfo,
                                            num_vertices);
        } else if (UNLIKELY(surfaceFlags & SITH_SURFACE_CEILING_SKY)) {
          sithRenderSky_TransformVertical(procEntry, &v65->surfaceInfo,
                                          sithRender_aVerticesTmp,
                                          num_vertices);
        }
      ...
      if (v76 > 0) {
        ...
        while (2) {
          ...
          if (rdGetVertexColorMode() == 0) {
            ...
            rdPrimit3_ClipFace(pSurfaceFrustum, v20->geometryMode,
                               v20->lightingMode, v20->textureMode,
                               &sithRender_idxInfo, &meshinfo_out,
                               &v65->surfaceInfo.face.clipIdk);
            /*rdPrimit3_NoClipFace( // pSurfaceFrustum,
                           v20->geometryMode,
            ...
            if ((v65->surfaceFlags & SITH_SURFACE_1000000) == 0) {
              ...
              v80[0] =
                  v65->surfaceInfo
                      .intensities[(v65->surfaceInfo.face.numVertices * 1) +
                                   v19];
              v80[1] =
                  v65->surfaceInfo
                      .intensities[(v65->surfaceInfo.face.numVertices * 1) +
              ...
              tmpGreen[0] =
                  v65->surfaceInfo
                      .intensities[(v65->surfaceInfo.face.numVertices * 2) +
                                   v19];
              tmpGreen[1] =
                  v65->surfaceInfo
                      .intensities[(v65->surfaceInfo.face.numVertices * 2) +
              ...
              tmpBlue[0] =
                  v65->surfaceInfo
                      .intensities[(v65->surfaceInfo.face.numVertices * 3) +
                                   v19];
              tmpBlue[1] =
                  v65->surfaceInfo
                      .intensities[(v65->surfaceInfo.face.numVertices * 3) +
            ...
            rdPrimit3_ClipFaceRGBLevel(pSurfaceFrustum, v20->geometryMode,
                                       v20->lightingMode, v20->textureMode,
                                       &sithRender_idxInfo, &meshinfo_out,
                                       &(v65->surfaceInfo).face.clipIdk);
          }
          ...
          rdCamera_pCurCamera->fnProjectLst(
              v20->vertices, sithRender_aVerticesTmp, meshinfo_out.numVertices);
          ...
          if (sithRender_lightingIRMode) {
            ...
            v20->ambientLight = stdMath_Clamp(
                level_idk->extraLight + sithRender_008d4098, 0.0, 1.0);
          }
          ...
          if (v20->ambientLight >= 1.0) {
            ...
            if (v20->light_level_static >= 1.0 && v68) {
              ...
              v20->lightingMode = RD_LIGHTMODE_NOTLIT;
                     ...
                     v20->lightingMode == RD_LIGHTMODE_GOURAUD) {
    ...
    // Surprisingly, this is a fairly minimal cost to the entire render, 3ms on
    // landing terminal spawn
    ...
    for (i = level_idk->thingsList; i; i = i->nextThing) {
      ...
      if (!((sithCamera_currentCamera->cameraPerspective & 0xFC) != 0 ||
            i != sithCamera_currentCamera->primaryFocus)) {
        continue;
      }
      ...
      rdMatrix_TransformPoint34(&i->screenPos, &i->position,
                                &rdCamera_pCurCamera->view_matrix);
      v63 = rdClip_SphereInFrustum(level_idk->clipFrustum, &i->screenPos,
                                   i->rdthing.model3->radius);
#ifdef SITHRENDER_SPHERE_TEST_SURFACES
      extern rdClipFrustum sithRender_absoluteMaxFrustum;

      if (UNLIKELY(v63 == SPHERE_CLIPPING_EDGE)) {
        v63 = rdClip_SphereInFrustum(&sithRender_absoluteMaxFrustum,
                                     &i->screenPos, i->rdthing.model3->radius);
      }
#endif
      ...
      if ((i->archlightIdx != -1) &&
        ...
        for (int k = 0; k < 4; k++) {
          for (int j = 0; j < iVar22->geosets[k].numMeshes; j++) {
            if (rdGetVertexColorMode() == 0) {
              iVar22->geosets[k].meshes[j].vertices_unk =
                  iVar22->geosets[k].meshes[j].vertices_i;
              iVar22->geosets[k].meshes[j].vertices_i =
                  sithWorld_pCurrentWorld->aArchlights[i->archlightIdx]
                      .aMeshes[j]
                      .aMono;
            } else {
              iVar22->geosets[k].meshes[j].paRedIntensities =
                  sithWorld_pCurrentWorld->aArchlights[i->archlightIdx]
                      .aMeshes[j]
                      .aRed;
              iVar22->geosets[k].meshes[j].paGreenIntensities =
                  sithWorld_pCurrentWorld->aArchlights[i->archlightIdx]
                      .aMeshes[j]
                      .aGreen;
              iVar22->geosets[k].meshes[j].paBlueIntensities =
                  sithWorld_pCurrentWorld->aArchlights[i->archlightIdx]
                      .aMeshes[j]
                      .aBlue;
            }
      ...
      if ((i->archlightIdx == -1) && (rdGetVertexColorMode() == 1)) {
        ...
        for (int k = 0; k < 4; k++) {
          for (int j = 0; j < iVar13->geosets[k].numMeshes; j++) {
            iVar13->geosets[k].meshes[j].paRedIntensities =
                iVar13->geosets[k].meshes[j].vertices_i;
            iVar13->geosets[k].meshes[j].paGreenIntensities =
                iVar13->geosets[k].meshes[j].vertices_i;
            iVar13->geosets[k].meshes[j].paBlueIntensities =
                iVar13->geosets[k].meshes[j].vertices_i;
          }
        }
      }
      ...
      if (((i->archlightIdx != -1) &&
           (i->rdthing.type == RD_THINGTYPE_MODEL)) &&
          (rdGetVertexColorMode() == 0)) {
        rdModel3 *iVar14 = i->rdthing.model3;
        for (int k = 0; k < 4; k++) {
          for (int j = 0; j < iVar14->geosets[k].numMeshes; j++) {
            iVar14->geosets[k].meshes[j].vertices_i =
                iVar14->geosets[k].meshes[j].vertices_unk;
          }
        }
      }
#endif
...
void sithRender_UpdateAllLights() {
  ...
  for (int j = 0; j < sithRender_numSectors; j++) {
    for (i = sithRender_aSectors[j]->adjoins; i; i = i->next) {
      if (i->sector->renderTick != sithRender_lastRenderTick &&
          (i->flags & 1) != 0) {
        // Allow things to peek their light around corners w/o screwing with
        // frustums
...
void sithRender_UpdateLights(sithSector *sector, flex_t prev, flex_t dist,
                             int depth) {
  sithThing *i;
  sithAdjoin *j;
  ...
  if (prev < 2.0 && sithRender_numLights < 0x20) {
    ...
    for (i = sector->thingsList; i; i = i->nextThing) {
      ...
      if ((i->thingflags & SITH_TF_LIGHT) &&
        ...
        if ((i->type == SITH_THING_ACTOR || i->type == SITH_THING_PLAYER) &&
            sithRender_numLights < 0x20) {
          // Actors all have a small amount of light
          if ((i->actorParams.typeflags & SITH_AF_FIELDLIGHT) &&
              i->actorParams.lightIntensity > 0.0) {
            rdMatrix_TransformPoint34(&vertex_out, &i->actorParams.lightOffset,
                                      &i->lookOrientation);
            rdVector_Add3Acc(&vertex_out, &i->position);

            sithRender_aLights[sithRender_numLights].intensity =
                i->actorParams.lightIntensity;
            rdCamera_AddLight(rdCamera_pCurCamera,
                              &sithRender_aLights[sithRender_numLights],
                              &vertex_out);
            ++sithRender_numLights;
          ...
          if (i->actorParams.timeLeftLengthChange > 0.0) {
            sithRender_aLights[sithRender_numLights].intensity =
                i->actorParams.timeLeftLengthChange;
            rdCamera_AddLight(rdCamera_pCurCamera,
                              &sithRender_aLights[sithRender_numLights],
                              &i->actorParams.saberBladePos);
            ++sithRender_numLights;
          }
        }
      }
  ...
  for (j = sector->adjoins; j; j = j->next) {
    if ((j->flags & 1) != 0 &&
        j->sector->renderTick != sithRender_lastRenderTick) {
...
void sithRender_RenderDynamicLights() {
  ...
  for (int k = 0; k < sithRender_numSectors; k++) {
    ...
    numSectorLights = 0;
    for (int i = 0; i < rdCamera_pCurCamera->numLights; i++) {
      // sithRender_RenderDebugLight(10.0,
      // &rdCamera_pCurCamera->lightPositions[i]);

      flex_t distCalc = rdVector_Dist3(&rdCamera_pCurCamera->lightPositions[i],
                                       &sectorIter->center);
      ...
      if (sithWorld_pCurrentWorld->alloc_unk9c[idx] !=
        ...
        for (int i = 0; i < numSectorLights; i++) {
          ...
          if (distCalc < tmpLights[i]->falloffMax)
            sithWorld_pCurrentWorld->verticesDynamicLight[idx] +=
                tmpLights[i]->intensity -
                distCalc * rdCamera_pCurCamera->attenuationMax;
...
void sithRender_RenderThings() {
  ...
  for (i = 0; i < sithRender_numSectors2; i++) {
    ...
    for (; thingIter; thingIter = thingIter->nextThing) {
      ...
      if ((thingIter->thingflags &
           (SITH_TF_DISABLED | SITH_TF_10 | SITH_TF_WILLBEREMOVED)) == 0 &&
          (thingIter->thingflags & SITH_TF_LEVELGEO) == 0 &&
          ((sithCamera_currentCamera->cameraPerspective & 0xFC) != 0 ||
           thingIter != sithCamera_currentCamera->primaryFocus)) {
        rdMatrix_TransformPoint34(&thingIter->screenPos, &thingIter->position,
                                  &rdCamera_pCurCamera->view_matrix);

        // printf("%f %f %f ; %f %f %f\n", thingIter->screenPos.x,
        // thingIter->screenPos.y, thingIter->screenPos.z,
        // thingIter->position.x, thingIter->position.y, thingIter->position.z);

        if (rdroid_curAcceleration > 0 ||
            thingIter->rdthing.type != RD_THINGTYPE_SPRITE3 ||
            sithRender_82F4B4 < 8) {
          // Allow things to peek their light around corners w/o screwing with
          // frustums
#ifdef QOL_IMPROVEMENTS
          if ((thingIter->thingflags & SITH_TF_LIGHT) != 0 &&
            ...
            rdCamera_SetAmbientLight(rdCamera_pCurCamera,
                                     stdMath_Clamp(thingIter->light, 0.0, 1.0));
          } else {
            rdCamera_SetAmbientLight(rdCamera_pCurCamera, a2);
          }
          ...
          case RD_THINGTYPE_MODEL:
            ...
            clipRadius = radius;
            clippingVal = rdClip_SphereInFrustum(
                v1->clipFrustum, &thingIter->screenPos, clipRadius);
            break;
          ...
          // printf("%f %f %f %d\n", (flex32_t)thingIter->screenPos.x,
          // (flex32_t)thingIter->screenPos.y, (flex32_t)thingIter->screenPos.z,
          // clippingVal);
          thingIter->rdthing.clippingIdk = clippingVal;
          if (clippingVal == SPHERE_FULLY_OUTSIDE ||
              sithRender_008d1668) // MoTS added: sithRender_008d1668
            continue;
          ...
          if (sithCamera_currentCamera->zoomScale != 1.0) {
            yval = sithCamera_currentCamera->invZoomScale *
                   (thingIter->screenPos).y;
          }
            ...
            case 3:
              if (yval < (flex_d_t)sithWorld_pCurrentWorld->lodDistance.x) {
                model3->geosetSelect = 0;
                         ...
                         (flex_d_t)sithWorld_pCurrentWorld->lodDistance.y) {
          ...
          if (yval >= (flex_d_t)curWorld->perspectiveDistance) {
            thingIter->rdthing.curTexMode = texMode > RD_TEXTUREMODE_AFFINE
                                                ? RD_TEXTUREMODE_AFFINE
                                                : texMode;
          } else
#endif
          {
          ...
          if (a2 >= 1.0) {
            if (v16) {
              lightMode = lightMode > RD_LIGHTMODE_FULLYLIT
                              ? RD_LIGHTMODE_FULLYLIT
                              : lightMode;
            } else {
              if (lightMode > RD_LIGHTMODE_DIFFUSE)
                lightMode = RD_LIGHTMODE_DIFFUSE;
            }
          } else if ((thingIter->thingflags & SITH_TF_IGNOREGOURAUDDISTANCE) ==
                         0 &&
                     yval >=
                         (flex_d_t)sithWorld_pCurrentWorld->gouradDistance) {
            if (lightMode > RD_LIGHTMODE_DIFFUSE)
              lightMode = RD_LIGHTMODE_DIFFUSE;
          } else {
...
int sithRender_RenderThing(sithThing *pThing) {
  ...
  if (!(pThing->thingflags & SITH_TF_INCAMFOV) &&
      !(g_debugmodeFlags &
        DEBUGFLAG_NOCLIP)) // Added: don't send sighted stuff in noclip
  {
  ...
  flex_t realDepth =
      pThing->screenPos.y - (pThing->rdthing.type == RD_THINGTYPE_MODEL
                                 ? pThing->rdthing.model3->radius
                                 : (flex_t)0.0);
  if (realDepth > 3.0) {
    skip_this_thing = 1;
  }
  ...
  if (!skip_this_thing) {
    ...
    if (sithRender_weaponRenderHandle &&
        (pThing->thingflags & SITH_TF_RENDERWEAPON)) {
  ...
  if (pThing->type == SITH_THING_EXPLOSION &&
      (pThing->explosionParams.typeflags &
       SITHEXPLOSION_FLAG_FLASH_BLINDS_THINGS)) {
    flex_t cameraDist = stdMath_Dist3D1(
        pThing->screenPos.x, pThing->screenPos.y, pThing->screenPos.z);
    ...
    flex_t flashMagnitude =
        ((flex_d_t)(flashB + flashR + flashG) * 0.013020833 -
         rdCamera_pCurCamera->attenuationMin * cameraDist) *
        0.1;
    if (flashMagnitude > 0.0) {
      sithPlayer_AddDyamicAdd(
          (__int64)((flex_d_t)flashR * flashMagnitude - -0.5),
          (__int64)((flex_d_t)flashG * flashMagnitude - -0.5),
          (__int64)((flex_d_t)flashB * flashMagnitude - -0.5));
    }
    pThing->explosionParams.typeflags &=
        ~SITHEXPLOSION_FLAG_FLASH_BLINDS_THINGS;
  }
...
void sithRender_RenderAlphaSurfaces() {
  ...
  for (int i = 0; i < sithRender_numSurfaces; i++) {
    ...
    // if (noDistCulling && !bIsSkySurface)
    {
      ...
      clipResult =
          rdClip_SphereInFrustum(pSphereFrustum, &centerTrans, v0->radius);
    ...
    // Added: Just in case
    if (!sithRender_idxInfo.vertexUVIdx &&
    ...
    if (clipResult != SPHERE_FULLY_INSIDE) {
#endif
      rdPrimit3_ClipFace(pSurfaceFrustum, v9->geometryMode, v9->lightingMode,
                         v9->textureMode, &sithRender_idxInfo, &meshinfo_out,
                         &v0->surfaceInfo.face.clipIdk);
#ifdef SITHRENDER_SPHERE_TEST_SURFACES
    } else {
      rdPrimit3_NoClipFace(/*pSurfaceFrustum,*/
                           v9->geometryMode, v9->lightingMode, v9->textureMode,
                           &sithRender_idxInfo, &meshinfo_out,
                           &v0->surfaceInfo.face.clipIdk);
    }
#endif
    ...
    rdCamera_pCurCamera->fnProjectLst(v9->vertices, sithRender_aVerticesTmp,
                                      meshinfo_out.numVertices);

    v9->ambientLight = stdMath_Clamp(
        surfaceSector->extraLight + sithRender_008d4098, 0.0, 1.0);

    // These light optimizations are for the sw renderer
#if !defined(TARGET_TWL) && !defined(TARGET_N64)
    if (v9->ambientLight < 1.0) {
      if (v9->lightingMode == RD_LIGHTMODE_DIFFUSE) {
        if (v9->light_level_static >= 1.0 &&
            surfaceSector->colormap == sithWorld_pCurrentWorld->colormaps) {
        ...
        } else if (surfaceSector->colormap !=
                   sithWorld_pCurrentWorld->colormaps) {
          v9->lightingMode = RD_LIGHTMODE_DIFFUSE;
...
void sithRender_WorldFlash(flex_t arg1, flex_t arg2) {
  if ((arg1 != 0.0) && ((uint16_t)((uint16_t)(arg2 < 0.0) << 8 |
                                   (uint16_t)(arg2 == 0.0) << 0xe) == 0)) {
