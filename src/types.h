...
// #include <nds.h>
#define flextov16(n)                                                           \
  ((v16)((int32_t)n.to_raw() >> (FIXED_POINT_DECIMAL_BITS - 12)))
#define flextot16(n)                                                           \
  ((v16)((int32_t)n.to_raw() >> (FIXED_POINT_DECIMAL_BITS - 4)))
#define flextof32(n)                                                           \
  ((int32_t)((int32_t)n.to_raw() >> (FIXED_POINT_DECIMAL_BITS - 12)))
#define f32toflex(n)                                                           \
  (numeric::fixed<FIXED_POINT_WHOLE_BITS, FIXED_POINT_DECIMAL_BITS>::          \
       from_base(n << (FIXED_POINT_DECIMAL_BITS - 12)))
...
#define flexdirect(n)                                                          \
  (numeric::fixed<FIXED_POINT_WHOLE_BITS,                                      \
                  FIXED_POINT_DECIMAL_BITS>::from_base(n))
#endif
...
typedef numeric::fixed<FIXED_POINT_WHOLE_BITS, FIXED_POINT_DECIMAL_BITS>
    flex_d_t;
#else
...
typedef int (*sithAICommandFunc_t)(sithActor *actor, sithAIClassEntry *a8,
                                   sithActorInstinct *a3, int32_t b,
                                   intptr_t a4);
typedef int (*sithControlEnumFunc_t)(int32_t inputFuncIdx,
                                     const char *pInputFuncStr, uint32_t a3,
                                     int32_t dxKeyNum, uint32_t a5, int32_t a6,
                                     stdControlKeyInfoEntry *pControlEntry,
                                     Darray *pDarr);
typedef int (*sithCollisionHitHandler_t)(sithThing *, sithSurface *,
...
typedef int (*sithCollision_collisionHandler_t)(sithThing *, sithThing *,
                                                sithCollisionSearchEntry *,
                                                int);
...
typedef void (*jkGuiDrawFunc_t)(jkGuiElement *, jkGuiMenu *, stdVBuffer *,
                                BOOL);
typedef int (*jkGuiEventHandlerFunc_t)(jkGuiElement *, jkGuiMenu *, int32_t,
                                       int32_t);
typedef int (*jkGuiClickHandlerFunc_t)(jkGuiElement *, jkGuiMenu *,
                                       int32_t mouseX, int32_t mouseY, BOOL);
